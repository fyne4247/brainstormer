...
echo ""
CURRENT_PROJECT="${NEW_PROJECT:-$PROJECT}"
for dir in */; do
  name="${dir%/}"
  [ -d "$name/.git" ] || continue
  [ -n "$(git -C "$name" status --porcelain 2>/dev/null)" ] || continue
  echo ""
  git -C "$name" diff --stat HEAD 2>/dev/null
  UNTRACKED="$(git -C "$name" ls-files --others --exclude-standard)"
  if [ -n "$UNTRACKED" ]; then
    echo "New file(s):"
    printf '  %s\n' "$UNTRACKED"
  fi
  if [ "$name" = "$CURRENT_PROJECT" ]; then
    ...
    read -r -p "\"$name\" has unsaved changes from an earlier session -- save it as well? [Y/n] " SAVE_ANSWER
  fi
  case "$SAVE_ANSWER" in
    [Nn]*) : ;;
    *)
